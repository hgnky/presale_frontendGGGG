...
export const getDepositAmount = async (contract) => {
  return new BigNumber(await contract.methods.totalPurchased().call())
}

export const deposit = async (contract, account, amount, signer) => {
  const presaleContract = contract.presale.contracts.PresaleErc20;
  return presaleContract.methods
      .purchaseaRIP()
      .send({ from: signer.account , value: new BigNumber(amount).times(new BigNumber(10).pow(18)).toString()})
...
export const claim = async (contract, account, amount, signer) => {
  const migrationContract = contract.presale.contracts.aRIPMigration;
  return migrationContract.methods
      .migrate(new BigNumber(amount).times(new BigNumber(10).pow(9)).toString())
      .send({ from: signer.account })
      .on('transactionHash', (tx) => {
        console.log(tx)
        return tx.transactionHash
      })
...
export const approveRIP = async (contract, account, address, amount, signer) => {
  const aRIPContract = contract.presale.contracts.aRIP;
  return aRIPContract.methods
      .approve(address, new BigNumber(amount).times(new BigNumber(10).pow(9)).toString())
      .send({ from: signer.account })
      .on('transactionHash', (tx) => {
        console.log(tx)
        return tx.transactionHash
